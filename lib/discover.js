...
import { listAllConversationsViaClaudeTab, withServiceTab } from './tab-fetch.js'
 ...
 *   allowOpenTab=false → the helper will refuse to spawn a background
 *   claude.ai tab if none is already open. Used by the auto-sync alarm.
 * @returns {Promise<{ signedIn: boolean, orgs: OrgSummary[], error?: string }>}
 */
export async function discoverClaude(opts = {}) {
  let orgs
  try {
  ...
  for (const org of orgs) {
    ...
    try {
      conversations = await listAllConversationsViaClaudeTab(
        ...
        { allowOpenTab: opts.allowOpenTab !== false }
      )
    } catch (err) {
      if (err && err.code === 'NO_TAB') {
        // Auto-sync wanted a passive run but the user has no claude.ai tab
        // open. Surface a soft signal so the alarm path can mark itself
        // skipped without showing an error to the user.
        return { signedIn: true, orgs: [], skipped: true, reason: 'no claude.ai tab open' }
      }
...
/**
 * @param {{ allowOpenTab?: boolean }} [opts] — see discoverClaude.
 * @returns {Promise<{ signedIn: boolean, conversations: ConvSummary[], error?: string }>}
 */
export async function discoverChatGPT(opts = {}) {
  // All chatgpt.com requests run inside a real chatgpt.com tab — direct
  // service-worker fetches are blocked by Cloudflare / CORS (same story as
  // claude.ai's /chat_conversations).
  try {
    return await withServiceTab(
      'chatgpt',
      { allowOpenTab: opts.allowOpenTab !== false },
      (fetchJson) => discoverChatGPTViaTab(fetchJson)
    )
  } catch (err) {
    if (err && err.code === 'NO_TAB') {
      // Passive run with no chatgpt.com tab open — soft-skip, not an error.
      return { signedIn: true, conversations: [], skipped: true, reason: 'no chatgpt.com tab open' }
    }
    return { signedIn: false, conversations: [] }
  }
}

async function discoverChatGPTViaTab(fetchJson) {
  let session
  try {
    session = await fetchJson(`${CHATGPT_ORIGIN}/api/auth/session`)
  } catch {
    return { signedIn: false, conversations: [] }
  }
  ...
  const conversations = items
    ...
    .map((m) => ({
      ...
      updatedAt: toEpochSec(m.update_time),
      vaultStatus: 'absent',
      cached: false,
    }))
...
// Claude timestamps are ISO strings; ChatGPT's list endpoint returns ISO
// strings too but its per-conversation endpoint returns epoch seconds —
// accept both shapes.
