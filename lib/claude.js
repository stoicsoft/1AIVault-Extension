 ...
 *   allowOpenTab=false → never spawn a hidden claude.ai tab; if none is open,
 *   the sync silently aborts. Used by the auto-sync alarm so background runs
 *   don't pop windows behind the user.
  ...
  for (const org of orgs) {
    ...
    try {
      ...
      if (err && err.code === 'NO_TAB') {
        // Passive (alarm) run with no claude.ai tab open — abort the whole
        // service cleanly. The caller treats this as "skipped".
        emit({ stage: 'done', service: 'claude', skipped: true, reason: 'no claude.ai tab open', imported: 0, signedIn: true })
